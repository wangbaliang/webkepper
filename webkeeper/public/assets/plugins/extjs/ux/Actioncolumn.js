/**
 * Created by wangyizhi on 2015/6/10.
 */
/**
 * @Description	文字 Action
 */
Ext.define('Ext.grid.column.Actiontextcolumn', {
    ...
    defaultRenderer: function(v, cellValues, record, rowIdx, colIdx, store, view) {
        ...
        for (; i < len; i++) {
            ...
            ret += '<a role="button" alt="' + (item.altText || me.altText) + '" '+
                '" class="' + me.actionIconCls + ' ' + prefix + 'action-col-' + String(i) + ' x-action-col-icon-my ' + (disabled ? prefix + 'item-disabled' : ' ') +
                (Ext.isFunction(item.getClass) ? item.getClass.apply(item.scope || scope, arguments) : (item.iconCls || me.iconCls || '')) + '"' +
